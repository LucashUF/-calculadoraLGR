...
```bash
git clone https://github.com/seu-usuario/lgr_rosa.git
cd lgr_rosa
```
...
- **`test_regression.py`** (124 testes)  
  Compara a implementação nova (`core/calculos.py`) com a implementação original (`reference/original_calculos.py`), garantindo equivalência em todos os cálculos.

- **`test_app_smoke.py`** (6 testes)  
  Verifica se a interface Streamlit executa sem exceções para casos típicos de uso.
